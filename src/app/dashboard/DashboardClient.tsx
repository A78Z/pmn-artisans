...
export default function DashboardClient({
    ...
    const { data: session } = useSession(); // We still use session for header info, but not for blocking
    ...
    // Initial check for new options (only if needed/filters changed drastically)
    ...
    const fetchData = useCallback(async () => {
        ...
        const fetchWithRetry = async (url: string, retries = 3, delay = 1000): Promise<Response> => {
            try {
                ...
                if (!res.ok && res.status !== 401 && retries > 0) throw new Error(`Fetch failed: ${res.status}`);
                return res;
            } catch (err: any) {
                if (retries > 0) {
        ...
        try {
            ...
            if (typeof window !== 'undefined') (window as any).__LAST_ERROR = error.message;
        } finally {
            setLoading(false);
        }
    }, [filters]);

    // Fetch data when filters change (Client-side)
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'transparent' }}>
            <style dangerouslySetInnerHTML={{
                ...
                @media (max-width: 1024px) {
                    ...
                    /* TABLE TO CARD TRANSFORMATION (Robust Block Method) */
                    table { 
            ...
            <header className="dashboard-header" style={{
                ...
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    <div className="hide-on-mobile" style={{ fontSize: '0.9rem', color: 'hsl(var(--muted-foreground))', fontWeight: '500', letterSpacing: '0.02rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        <span>{(user as any)?.chambre || "Chambre des Métiers"}</span>
                    </div>
                    <button
                        className="btn"
                ...
                <main id="main-scroll-container" className="dashboard-main" style={{ flex: 1, padding: '2rem 3rem', overflowY: 'auto', backgroundColor: 'transparent' }}>
                    ...
                    <div className="highlight-zone">
                        ...
                        <div style={{ overflowX: 'visible' }}>
                            <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '0 0.75rem', fontSize: '0.95rem' }}>
                                <thead>
                                    ...
                                    {data.map((artisan) => (
                                        ...
                                        >
                                            <td style={{ padding: '1.25rem 1.5rem', fontWeight: '600', borderTopLeftRadius: 'var(--radius)', borderBottomLeftRadius: 'var(--radius)', color: 'hsl(var(--foreground))' }}>
                                                <span className="mobile-label">Artisan</span>
                                            ...
                                            <td style={{ padding: '1.25rem 1rem', color: 'hsl(var(--muted-foreground))', fontWeight: '500' }}>
                                                <span className="mobile-label">Métier</span>
                                                <span>{artisan.metier}</span>
                                            </td>
                                            <td style={{ padding: '1.25rem 1rem' }}>
                                                <span className="mobile-label">Filière</span>
                                                <span style={{
                                            ...
                                            <td style={{ padding: '1.25rem 1rem' }}>
                                                <span className="mobile-label">Localisation</span>
                                                <div style={{ textAlign: 'right' }}>
                                                    ...
                                                    <div style={{ fontSize: '0.8rem', color: 'hsl(var(--muted-foreground))', marginTop: '0.3rem', display: 'flex', alignItems: 'center', gap: '0.3rem', justifyContent: 'flex-end' }}>
                                                        <span className="hide-on-mobile" style={{ width: '4px', height: '4px', borderRadius: '50%', backgroundColor: 'hsl(var(--muted-foreground))' }}></span>
